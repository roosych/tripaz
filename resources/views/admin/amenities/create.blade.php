...
@section('title', 'Добавить удобство')
...
<div class="page-header d-flex align-items-center justify-content-between flex-wrap gap-2">
    <div>
        <h1 class="h4 fw-bold mb-0">
            <i class="ph ph-squares-four me-2" style="color:#dc3545"></i>Добавить удобство
        </h1>
        <p class="text-muted mb-0 small mt-1">Создайте новое удобство или функцию для объявлений</p>
    </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4">
                ...
                <form method="POST" action="{{ route('admin.amenities.store') }}">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <select id="listing_type"
                                ...
                                @foreach($types as $listingType)
                                    <option value="{{ $listingType->value }}"
                                            {{ old('listing_type') === $listingType->value ? 'selected' : '' }}>
                                        {{ $listingType->label() }}
                                    </option>
                                @endforeach
                            </select>
                            @error('listing_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">Оставьте пустым, чтобы удобство было доступно для всех типов объявлений.</div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <select id="amenity_group_id"
                                    ...
                                    @foreach($groups as $g)
                                        <option value="{{ $g->id }}" {{ old('amenity_group_id') == $g->id ? 'selected' : '' }}>
                            ...
                            @error('amenity_group_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">Необязательно. <a href="{{ route('admin.amenity-groups.index') }}" target="_blank">Управление группами →</a></div>
                        </div>
                        <div class="col-md-4">
                    ...
                    {{-- Icon preview --}}
                    <div id="icon-preview" class="mb-4" style="display:none">
                        ...
                        <div class="d-flex align-items-center gap-2 p-3 bg-light rounded border">
                            <i id="preview-icon" class="fs-3 text-primary"></i>
                            <span id="preview-icon-name" class="text-muted small"></span>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-2">
                        ...
                        <button type="submit" class="btn btn-danger">
                            <i class="ph ph-plus-circle me-1"></i>Создать удобство
                        </button>
                    </div>
                </form>
